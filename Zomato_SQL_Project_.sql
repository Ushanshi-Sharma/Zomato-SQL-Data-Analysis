...
-- 5. Find the user id and max amount of order placed by user at once.
SELECT user_id, amount
FROM orders
GROUP BY user_id, amount
HAVING amount = (select MAX(amount) from orders);

-- 6. Find all the unique restaurants name available on Zomato
SELECT DISTINCT(r_name)
FROM restaurants;

-- 7. Show all unique food items making different category for veg and non-veg
SELECT DISTINCT(f_name), type
FROM food
GROUP BY type, f_name
ORDER BY type ;
...
-- 5. Show all orders with order details for a particular customer in a particular date range (Ankit , 10June- 10July)
